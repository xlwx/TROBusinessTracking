...
$addJS = "
	$(function() {
	    ...
	    $(\"#search\").click(function() {
	        ...
	        if(searchString) {
	            ...
	            $.ajax({
	                type: \"GET\",
					url: '".AJAX_URL."/troCruiseNews_do_search.php',	              
	                data: data,
    ...
    function delRow(){
			if (confirm('Are you sure that you want to delete it?')==true)
		   {
				...
				$.ajax({
					type: 'GET',
					url: '".AJAX_URL."/troCruiseNews-delete.php',
					data: data,
		...
		function add(){
			 window.location.href='".BASE_URL."troCruiseNews-add.php';
		}         
\n";

$addJavaScript[] = templateAddJavaScript($addJS);

$pageTitle = "TRO Cruise News-List";
...
<div class="list-group animated bounceInDown"  id="allresult">
	...
	<?php
	$db->query("select * from TROC order by BannerID");
	$db->execute();
	$row = $db->resultset();

	echo "<div class='panel panel-default'>";
	echo "<div class='panel-heading'>TRO CruiseNews</div>";
	...
	while($j<count($row)){
		...
		echo "<tr>";
			...
			echo "<td>";
    		echo "<a href='troCruiseNews-edit.php?campaignName={$n}'>";
