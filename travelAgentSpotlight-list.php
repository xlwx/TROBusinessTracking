...
$addJS = "
	$(function() {
	    ...
	    $(\"#search\").click(function() {
	        ...
	        if(searchString) {
	            ...
	            $.ajax({
	                type: \"GET\",
					url: '".AJAX_URL."/travelAgentSpotlight_do_search.php',	              
	                data: data,
    ...
    function delRow(){
			if (confirm('Are you sure that you want to delete it?')==true)
		   {
				...
				$.ajax({
					type: 'GET',
					url: '".AJAX_URL."/travelAgentSpotlight-delete.php',
					data: data,
		...
		function add(){
			 window.location.href='".BASE_URL."travelAgentSpotlight-add.php';
		}         
\n";

$addJavaScript[] = templateAddJavaScript($addJS);

$pageTitle = "Travel Agent Spotlight List";
...
<div class="list-group animated bounceInDown"  id="allresult">
	...
	<?php
	$db->query("select * from travelAgentSpotlight order by BannerID");
	$db->execute();
	$row = $db->resultset();
	...
	while($j<count($row)){
		...
		echo "<tr>";
			...
			echo "<td>";
    		echo "<a href='travelAgentSpotlight-edit.php?campaignName={$n}'>";
